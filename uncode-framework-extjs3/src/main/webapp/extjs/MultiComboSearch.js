Ext.form.MultiComboSearch = Ext.extend(Ext.form.TriggerField, {
	...
	initList : function() {
		if (!this.list) {
			...
			if (!this.tpl) {
				// alert(cls);
				// x-combo-list-item
				this.tpl = '<tpl for="."><div class="' + cls + '-item" ext:qtip="{' + this.displayField + '}"><span class="unchecked" id="checkBox_' + this.id
	...
	/**
	 * zKF36477 为了彻底清空下拉多选框所添加的一个清空方法
	 *
	...
	/**************zKF36477 为了彻底清空下拉多选框所添加的一个清空方法 End*********************/
	setValue : function(v) {
		var text = v;
		if (this.valueField) {
